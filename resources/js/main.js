                ...
                function generateCertificationDiv(opts, lang) {
                    ...
                    if ($var.hasClass('complete')) {
                        ...
                        if ($var.hasClass('learning')) {
                            ...
                            if (price == 0) {
                                text += lang.cert_free_again;
                            } else {
                                text += lang.cert_paid_again + price;
                            }
                            text += '</span>';
                        }
                    } else {
                    ...
                    success: function(data) {
                        ...
                        $.getJSON('json/certs.json', function(data) {
                            ...
                            $.each(data, function(key) {
                                html += generateCertificationDiv({
                                    cert: key,
                                    name: data[key][0]['name'],
                                    desc: data[key][0]['desc'],
        ...
        if ($('body').hasClass('internet') && !($('body').hasClass('history'))) {
            ...
            if ($('body').hasClass('navigate')) {
                ...
                if ($('body').hasClass('tab-hack')) {
                    $('.nav-tabs .active').next().next().children().css({
                        'color': 'red',
                        'font-weight': 'bold'
                    });
                    $.ajax({
                    ...
                    $('.nav-tabs .active').next().children().css({
                        'color': 'red',
                        'font-weight': 'bold'
                    });
                    $.ajax({
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        openModal({
                            title: title,
                            text: text,
                            btn: '<input type="submit" data-dismiss="modal" class="btn btn-info" value="Ok">',
                            input: ''
                        })
                    }
                });
                    ...
                    if (common[0].unread > 0) {
                        ...
                        if (!$('body').hasClass('mail')) {
                            if (!$('#notify').hasClass('shown')) {
                                gritterNotify({
                                    title: common[0].unread_title,
                                    text: common[0].unread_text,
                                    img: '',
                                    sticky: false
                                })
...
$('#sidebar a').mouseenter(function() {
	...
	if($( 'link[href*="'+ link + '"]' ).length == 0){
		$('<link rel="prefetch" href="'+ link +'">').appendTo("body");
    	$('<link rel="prefetch" href="/static'+ link +'">').appendTo("body");
    }
});
